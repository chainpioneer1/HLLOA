...
(function ($) {
    ...
    var $doc = $(document),
        ...
        Template = $('<div class="rangePicker">' +
            ...
            '<footer>' +
            '<div>您可以选择一个日期或时间段</div>' +
            '<button type="button" class="confirm btn-rect btn-blue">确定</button>' +
    ...
    RangePicker.prototype = {
        ...
        init: function () {
            ...
            this.picker.add(this.obj).on('mousedown', function (e) {
                e.stopPropagation()
            });
            ...
            for (i = this.settings.presets.length; i--;) {
                presetWrap.prepend("<button type='button' value='" +
                    this.settings.presets[i].value + "'>" +
                    this.settings.presets[i].buttonText + "</button>");
            }
        ...
        bind: function () {
            ...
            this.obj.on('click', function (e) {
                ...
                // window.scroll(0, $(window).scrollTop() - 1);
                if (isFirstClicked) {
                    if (that.settings.setDate) {
                        var _setDate = that.settings.setDate;
                        that.picker.find('.calendar.from select').val(_setDate[0][1]);
                        $(that.picker.find('.calendar.from .months button')[_setDate[0][0] - 1]).click();
                        that.picker.find('.calendar.to select').val(_setDate[1][1]);
                        $(that.picker.find('.calendar.to .months button')[_setDate[1][0] - 1]).click();
                    }
                    isFirstClicked = false;
                }

                setTimeout(function () {
                    that.tether = new Tether({
                        element: that.picker,
                        target: that.obj,
                        attachment: that.settings.RTL ? 'top right' : 'top left',
                        targetAttachment: that.settings.RTL ? 'bottom right' : 'bottom left',
                        constraints: [{
                            to: 'window',
                            attachment: 'together'
                        }]
        ...
        changeMonth: function (calendarIdx, monthIdx) {
            ...
            if (this.result[0][1] == this.result[1][1] && calendarIdx == 0) {
                this.picker.find('.calendar.to')
                    .find('button').prop('disabled', false)
                    .slice(0, monthIdx).prop('disabled', true)
        ...
        displayValue: function (format) {
            ...
            format = format.replace('%S', this.result[0][1] + '年' + this.settings.months[this.result[0][0] - 1]);
            format = format.replace('%E', this.result[1][1] + '年' + this.settings.months[this.result[1][0] - 1]);
    ...
    $.fn.rangePicker.defaults = {
        ...
        minDate: [12, 2019],
        maxDate: [12, 2099],
        setDate: [[new Date().getMonth() + 1, new Date().getFullYear()], [new Date().getMonth() + 1, new Date().getFullYear()]]
    };
