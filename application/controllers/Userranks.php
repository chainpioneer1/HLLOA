...
class Userranks extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->mainModel = $this->userrank_m;
    }
    ...
    public function manage()
    {
        ...
        $this->data['title'] = '行政管理 ＞ 职级管理';
        $this->data["subscript"] = "settings/script";
        $this->data["subcss"] = "settings/css";
        $this->data['apiRoot'] = $apiRoot = 'userranks/manage';
        $this->data['mainModel'] = 'tbl_user_rank';
    ...
    public function output_content($items, $startNo = 0)
    {
        ...
        foreach ($items as $unit):
            ...
            $output .= '<td>' . $unit->standard_factor . '</td>';
            $output .= '<td>' . $unit->rank_factor . '</td>';
            $output .= '<td>' . $unit->gangwei_price . '</td>';
            $output .= '<td>' . $unit->jixiao_price . '</td>';
            $output .= '<td>';
    ...
    public function updateItem()
    {
        ...
        if ($_POST) {
            $editArr = array(
                ...
                'gangwei_price' => $this->input->post('gangwei_price'),
                'jixiao_price' => $this->input->post('jixiao_price'),
                'standard_factor' => $this->input->post('standard_factor'),
                'rank_factor' => $this->input->post('rank_factor'),
                'status' => 1,
